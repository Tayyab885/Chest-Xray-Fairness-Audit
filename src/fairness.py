...
def subgroup_auroc(y_true, y_prob, metas, labels, group_key):
...
def youden_threshold(y_true_col, y_prob_col):
    fpr, tpr, thr = roc_curve(y_true_col, y_prob_col)
    return float(thr[np.argmax(tpr - fpr)])

def equalized_odds(y_true, y_prob, metas, label_idx, group_key, threshold):
    groups = sorted({m[group_key] for m in metas})
    res, tprs, fprs = {}, [], []
    for g in groups:
        mk = _mask(metas, group_key, g)
