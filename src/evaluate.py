...
def compute_auroc(y_true: np.ndarray, y_prob: np.ndarray, labels: list) -> dict:
	"""Compute per-label AUROC and macro average."""
	out = {}
	scores = []
...
def predict(model, loader, device) -> tuple:
	"""Run model on loader; return (y_true, y_pred, metadata_list)."""
	model.eval()
	ys, ps, metas = [], [], []
	with torch.no_grad():
